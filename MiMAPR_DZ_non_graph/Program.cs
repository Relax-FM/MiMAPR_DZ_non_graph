...
namespace MiMAPR_DZ_non_graph
{
    class Programm
    {
        static void Main(string[] args)
        {
            ...
            ElemsFab.AddElem("C", "C20", 1, 2, new Dictionary<string, double>() { { "value", 1.0 } });
            ...
            ElemsFab.AddElem("C", "Cb1", 2, 3, new Dictionary<string, double>() { { "value", 2.0 } });
            ElemsFab.AddElem("I", "Id1", 2, 3, new Dictionary<string, double>() { { "value", 0.0 } });
            ...
            ElemsFab.SetCountingTimeSettings(0, 1e-3, 1e-8); // Временные настройки моделирования
            ElemsFab.SetCountingEpsilonSettings(1e-2,1e-6); // Критерии останова (TODO: Нихуя не понял нужны ли они)
            ElemsFab.SetStartPhi(); // Просто создаем начальные пхи 
            ElemsFab.ClearMatrixAndVector(); // Выделяем память и заполняем нулями вектор и матрицу

            ElemsFab.CreateTestMatrix(); // Заполняем вектор инфой 
            ElemsFab.CreateTestVector(); // Заполняем матрицу инфой

            ElemsFab.Calculating(); // Тут вся магия по идее происходить будет когда-нибудь
        }
    }
}